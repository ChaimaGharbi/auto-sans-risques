...
export function ArticlesFetchDialog() {
  ...
  const articlesUIProps = useMemo(() => {
    return {
      ...
      showfetchModelsDialog: articlesUIContext.showfetchModelsDialog,
      closefetchModelsDialog: articlesUIContext.closefetchModelsDialog,
    };
  }, [articlesUIContext]);

  const { articles } = useSelector(
    (state) => ({
      articles: selectedArticles(state.models.entities, articlesUIProps.ids),
    }),
    shallowEqual
  );
  ...
  useEffect(() => {
    if (!articlesUIProps.ids || articlesUIProps.ids.length === 0) {
      articlesUIProps.closefetchModelsDialog();
    }
  ...
  return (
    <Modal
      show={articlesUIProps.showfetchModelsDialog}
      onHide={articlesUIProps.closefetchModelsDialog}
    ...
    >
      ...
      <Modal.Footer>
        <div>
          <button
            type="button"
            onClick={articlesUIProps.closefetchModelsDialog}
