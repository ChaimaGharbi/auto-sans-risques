...
import * as uiHelpers from "./ArticlesUIHelper";
...
import { TypeColumnFormatter } from "../categories-table/column-formatters";
...
export function ArticlesTable() {
  ...
  const articlesUIProps = useMemo(() => {
    return {
      ...
      openDeleteArticleDialog: articlesUIContext.openDeleteArticleDialog,
    };
  }, [articlesUIContext]);

  // Getting curret state of categorys list from store (Redux)
  const { currentState } = useSelector(
    (state) => ({ currentState: state.articles }),
    shallowEqual
  );
  ...
  useEffect(() => {
    ...
    dispatch(
      actions.fetchArticles(articlesUIProps.queryParams, articlesUIProps.categoryId)
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      formatExtraData: {
        ...
        openDeleteArticleDialog: articlesUIProps.openDeleteArticleDialog,
      },
