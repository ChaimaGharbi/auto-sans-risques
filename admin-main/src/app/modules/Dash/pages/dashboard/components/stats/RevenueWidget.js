...
export function RevenueWidget({ className,revenus,revenusData }) {
  ...
  useEffect(() => {
    ...
    const options = getChartOption(layoutProps,revenusData);
    const chartRedeems = new ApexCharts(element, options);
    chartRedeems.render();
    return function cleanUp() {
      chartRedeems.destroy();
    };
  }, [layoutProps]);
      ...
      <div className="card-body d-flex flex-column p-0">
        <div className="d-flex align-items-center justify-content-between card-spacer flex-grow-1">
          ...
          <div className="d-flex flex-column text-right">
            ...
            <span className="text-muted font-weight-bold mt-2">Revenus</span>
...
function getChartOption(layoutProps,revenusData) {
  var options = {
    series: [
      {
        name: "Revenus",
    ...
    stroke: {
      ...
      show: true,
      width: 3,
      colors: [layoutProps.colorsGrayGray300]
    },
    xaxis: {
    ...
    yaxis: {
      min: Math.min(...revenusData),
      max: Math.max(...revenusData)+5,
      labels: {
        show: false,
        style: {
          colors: layoutProps.colorsGrayGray500,
