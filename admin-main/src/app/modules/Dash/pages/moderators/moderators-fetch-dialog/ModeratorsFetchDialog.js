import React, { useEffect, useMemo } from "react";
import { Modal } from "react-bootstrap";
import { shallowEqual, useSelector } from "react-redux";
import { ModeratorStatusCssClasses } from "../ModeratorsUIHelpers";
import { useModeratorsUIContext } from "../ModeratorsUIContext";

const selectedModerators = (entities, ids) => {
  const _moderators = [];
  ids.forEach((id) => {
    const moderator = entities.find((el) => el.id === id);
    if (moderator) {
      _moderators.push(moderator);
    }
  });
  return _moderators;
};

export function ModeratorsFetchDialog({ show, onHide }) {
  // ModeratorsUI Context
  const moderatorsUIContext = useModeratorsUIContext();
  const moderatorsUIProps = useMemo(() => {
    return {
      ids: moderatorsUIContext.ids,
    };
  }, [moderatorsUIContext]);

  // ModeratorsRedux state
  const { moderators } = useSelector(
    (state) => ({
      moderators: selectedModerators(
        state.moderators.entities,
        moderatorsUIProps.ids
      ),
    }),
    shallowEqual
  );

  // if moderators weren't selected we should close modal
  useEffect(() => {
    if (!moderatorsUIProps.ids || moderatorsUIProps.ids.length === 0) {
      onHide();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [moderatorsUIProps.ids]);

  return (
    <Modal
      show={show}
      onHide={onHide}
      aria-labelledby="example-modal-sizes-title-lg"
    >
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">
          Fetch selected elements
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <div className="timeline timeline-5 mt-3">
          {moderators.map((moderator) => (
            <div
              className="timeline-item align-items-start"
              key={`id${moderator.id}`}
            >
              <div className="timeline-label font-weight-bolder text-dark-75 font-size-lg text-right pr-3" />
              <div className="timeline-badge">
                <i
                  className={`fa fa-genderless text-${
                    ModeratorStatusCssClasses[moderator.isActive]
                  } icon-xxl`}
                />
              </div>
              <div className="timeline-content text-dark-50 mr-5">
                <span
                  className={`label label-lg label-light-${
                    ModeratorStatusCssClasses[moderator.isActive]
                  } label-inline`}
                >
                  ID: {moderator.id}
                </span>
                <span className="ml-3">{moderator.body}</span>
              </div>
            </div>
          ))}
        </div>
      </Modal.Body>
      <Modal.Footer>
        <div>
          <button
            type="button"
            onClick={onHide}
            className="btn btn-light btn-elevate"
          >
            Cancel
          </button>
          <> </>
          <button
            type="button"
            onClick={onHide}
            className="btn btn-primary btn-elevate"
          >
            Ok
          </button>
        </div>
      </Modal.Footer>
    </Modal>
  );
}
