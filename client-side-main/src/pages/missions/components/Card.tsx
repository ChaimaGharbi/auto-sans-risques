...
const Card = ({ id, comment, date, client, expertId }) => {
  ...
  return (
    <div className="flex md:items-center p-2 justify-between flex-col md:flex-row ">
      ...
      <div className="ml-auto grid gap-3 w-full xs:w-auto">
        <button
          className="w-full xs:w-44 py-3 rounded-lg bg-[#0CD45C] text-white font-black font-rubik"
          style={{
            boxShadow: '0px 7px 34px rgba(78, 173, 255, 0.3)',
          }}
          disabled={accept.loading}
          onClick={() => acceptMission(id, expertId)}
        >
          VALIDER
        </button>
        <button
          className="w-full xs:w-44 py-3 rounded-lg bg-[#FF6D64] text-white font-black font-rubik"
          style={{
            boxShadow: '0px 7px 34px rgba(78, 173, 255, 0.3)',
          }}
          disabled={reject.loading}
          onClick={() => rejectMission(id, expertId)}
        >
          DECLINER
        </button>
      </div>
